		...
		<>
			...
			<nav className="bg-custom-blue-1 text-white px-4 py-3 shadow-md flex justify-between items-center">
				...
				<div className="hidden sm:flex items-center space-x-6">
					<span className="capitalize">{usuario?.nombre_completo || "Usuario"}</span>
					<button
			...
			<div className={`fixed top-0 left-0 h-full w-64 bg-custom-blue-1 text-white shadow-lg transform ${menuAbierto ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-300 z-50 sm:hidden`}>
				...
				<ul className="flex flex-col p-4 space-y-4">
					<li className="capitalize">{usuario?.nombre_completo || "Usuario"}</li>
					<li>
						<button
							onClick={handleLogout}
