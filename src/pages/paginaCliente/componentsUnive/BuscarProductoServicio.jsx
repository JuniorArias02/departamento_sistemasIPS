...
import { buscarProductoServicio } from "../../../services/inventario_services";
import {
	Search,
	Package,
	Barcode,
...
const BuscarProductoServicio = ({
	...
	useEffect(() => {
		...
		const buscar = async () => {
			...
			try {
				const data = await buscarProductoServicio(debouncedQuery);
			...
			{isOpen && debouncedQuery.length >= 2 && resultados.length === 0 && !loading && (
				<div className="absolute z-10 w-full mt-1 bg-white border rounded-xl shadow-lg p-4">
					<div className="text-center text-gray-500 py-2">
						...
						<p className="text-xs">Intenta con otro código o nombre</p>
					</div>
				</div>
			)}
				...
				<div className="mt-2 p-3 bg-green-50 border border-green-200 rounded-lg">
					<div className="flex items-center">
						...
						<span className="text-sm text-green-700">
							Seleccionado: <strong>{selectedItem.nombre}</strong> (Código: {selectedItem.codigo_producto})
						</span>
					</div>
				</div>
			)}
...
export default BuscarProductoServicio;
