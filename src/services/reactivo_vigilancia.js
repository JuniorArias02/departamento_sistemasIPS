...
import { CREAR_REACTIVO_VIGILANCIA,LISTAR_REACTIVO_VIGILANCIA,ELIMINAR_REACTIVO_VIGILANCIA,EXPORTAR_REACTIVO_VIGILANCIA } from "../const/url"
...
export const listarReactivosVigilancia = async () => {
  try {
    ...
    throw new Error(error?.response?.data?.mensaje || "Error al listar  reactivos de vigilancia");
  }
}
...
export const eliminarReactivoVigilancia = async (id) => {
  try {
    ...
    throw new Error(error?.response?.data?.error || "Error al eliminar el reactivo");
  }
};
