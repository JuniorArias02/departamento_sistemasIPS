...
import { CREAR_EQUIPO_BIOMEDICO, LISTAR_EQUIPO_BIOMEDICO, ELIMINAR_EQUIPO_BIOMEDICO,EXPORTAR_EQUIPO_BIOMEDICO } from "../const/url"
...
export const listarEquiposBiomedicos = async () => {
  try {
    ...
    throw new Error(error?.response?.data?.mensaje || "Error al listar los equipos biomedicos");
  }
}
...
export const eliminarEquipoBiomedico = async (id) => {
  try {
    ...
    throw new Error(error?.response?.data?.error || "Error al eliminar el equipo");
  }
};
