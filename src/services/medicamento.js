...
import { CREAR_MEDICAMENTO,LISTAR_MEDICAMENTO,ELIMINAR_MEDICAMENTO, EXPORTAR_MEDICAMENTO } from "../const/url"
...
export const listarMedicamentos = async () => {
  try {
    ...
    throw new Error(error?.response?.data?.mensaje || "Error al listar medicamentos");
  }
}
...
export const eliminarMedicamento = async (id) => {
  try {
    ...
    throw new Error(error?.response?.data?.error || "Error al eliminar el medicamento");
  }
};
